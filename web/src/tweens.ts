 ...
 * 헬퍼 함수: 지연 후 콜백
 */
export function tweenDelay(duration: number, onComplete: () => void): Tween {
  return tweenManager.create({
    from: 0,
    to: 1,
    duration,
 ...
 * 헬퍼 함수: 순차 애니메이션
 */
export function tweenSequence(tweens: Array<() => Tween>): void {
 ...
 * 헬퍼 함수: 병렬 애니메이션
 */
export function tweenParallel(tweens: Array<() => Tween>, onAllComplete?: () => void): void {
  if (tweens.length === 0) {
