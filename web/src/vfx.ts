...
class ParticleEmitter {
   ...
   * 폭발 이펙트
   */
  burst(x: number, y: number, count: number, color: number, options?: {
    speed?: number;
    size?: number;
    life?: number;
    gravity?: number;
   ...
   * 원뿔형 분출
   */
  spray(x: number, y: number, count: number, angle: number, spread: number, color: number, options?: {
    speed?: number;
    size?: number;
    life?: number;
  }) {
   ...
   * 떨어지는 이펙트
   */
  rain(x: number, y: number, count: number, color: number, options?: {
    width?: number;
    speed?: number;
    size?: number;
    life?: number;
   ...
   * 번쩍임 효과
   */
  flash(x: number, y: number, color: number, size: number = 50) {
    const sprite = new Graphics();
    sprite.circle(0, 0, size);
   ...
   * 상승하는 이펙트 (회복, 버프 등)
   */
  rise(x: number, y: number, count: number, color: number, options?: {
    speed?: number;
    size?: number;
    life?: number;
  }) {
...
class VFXManager {
   ...
   * 피해 이펙트 (빨간 번쩍임 + 폭발) 💥 강화!
   */
  playDamageEffect(x: number, y: number, value: number) {
   ...
   * 회복 이펙트 (초록 반짝임 + 상승) ✨ 강화!
   */
  playHealEffect(x: number, y: number, value: number) {
    if (!this.emitter) return;
   ...
   * 보호막 이펙트 (파란 폭발) 🛡️ 강화!
   */
  playShieldEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   */
  playEnergyEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 드로우 이펙트 (청록 번쩍임) 🃏 강화!
   */
  playDrawEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 화상 이펙트 (불꽃) 🔥 강화!
   */
  playBurnEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 빙결 이펙트 (얼음 조각) ❄️ 강화!
   */
  playFreezeEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 감전 이펙트 (번개) ⚡ 강화!
   */
  playShockEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 취약 이펙트 (보라색 오라) 💀 강화!
   */
  playVulnerableEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 버프 이펙트 (금색 반짝임) ⭐ 강화!
   */
  playBuffEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 승리 이펙트 (금색 불꽃놀이) 🎉 강화!
   */
  playVictoryEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 패배 이펙트 (회색 낙하) 💀 강화!
   */
  playDefeatEffect(x: number, y: number) {
    if (!this.emitter) return;
   ...
   * 카드 사용 트레일 (궤적) ✨ 강화!
   */
  playCardTrailEffect(x: number, y: number, color: number) {
    if (!this.emitter) return;
