...
const cloneCard = (card: Card, suffix: number): Card => ({
  ...card,
...
describe('Card Priority System', () => {
  ...
  it('should resolve cards in correct priority order', () => {
    ...
    if (specialIdx >= 0) store.declareCard(specialIdx);
    if (attackIdx >= 0) store.declareCard(attackIdx);
    if (defenseIdx >= 0) store.declareCard(defenseIdx);
    if (healIdx >= 0) store.declareCard(healIdx);
