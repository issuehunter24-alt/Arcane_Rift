...
type AuthState = {
  ...
  refreshProfileNickname: () => Promise<void>;
...
function resolveSessionNickname(session: Session | null): string | null {
  if (!session) {
    return null;
  }
  ...
  return (
    normalizeNickname(metaRecord.display_name as string | undefined) ??
    normalizeNickname(metaRecord.nickname as string | undefined) ??
    null
  );
}
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  async refreshProfileNickname() {
    ...
    const profileNickname =
      normalizeNickname(data?.display_name) ??
      normalizeNickname((data as { nickname?: string } | null | undefined)?.nickname) ??
      sessionNickname ??
      defaultNickname;
    set({ profileNickname });
  },
