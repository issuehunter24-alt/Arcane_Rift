...
let currentEnemyPortrait: string | null = null;
  ...
  function showCardPreview(card: typeof store.hand[0]) {
    ...
    if (card.effectText && card.effectText.trim().length > 0) {
      ...
      const effectsText = card.effects.map(eff => {
        if (!eff) return '';
        if (eff.type === 'Damage') return `🗡️ 피해 ${eff.value}${eff.aoe ? ' (광역)' : ''}`;
        if (eff.type === 'Heal') return `💚 회복 ${eff.value}${eff.aoe ? ' (광역)' : ''}`;
        if (eff.type === 'Shield') return `🛡️ 보호막 ${eff.value}`;
        if (eff.type === 'Guard') return `🛡️ 가드 ${eff.value}`;
        if (eff.type === 'Draw') return `📇 드로우 ${eff.value}장`;
        if (eff.type === 'GainAction') return `⚡ 에너지 +${eff.value}`;
  ...
  function formatStatus(status: typeof playerStatus): string {
    ...
    // 보호 효과 (파란색)
    if (status.shield > 0) badges.push(`<span class="status-badge status-defense">🛡️ ${status.shield}<sub>${status.shieldDuration}T</sub></span>`);
    if (status.guard > 0) badges.push(`<span class="status-badge status-defense">🛡️ 가드 ${status.guard}<sub>${status.guardDuration}T</sub></span>`);
    if (status.evasionCharges > 0) badges.push(`<span class="status-badge status-defense">💨 ${status.evasionCharges}회<sub>${status.evasionDuration}T</sub></span>`);
    if (status.nullifyCharges > 0) badges.push(`<span class="status-badge status-defense">🚫 ${status.nullifyCharges}</span>`);
    if (status.counterValue > 0) badges.push(`<span class="status-badge status-buff">⚔️ 반격 ${status.counterValue}<sub>${status.counterDuration}T</sub></span>`);
    if (status.immuneKeywords.length > 0) badges.push(`<span class="status-badge status-defense">🛡️ ${status.immuneKeywords.join(',')}<sub>${status.immuneDuration}T</sub></span>`);
    
    // 버프 효과 (초록색)
    if (status.attackBuff > 0) badges.push(`<span class="status-badge status-buff">⚔️ +${status.attackBuff}%</span>`);
    if (status.regen > 0) badges.push(`<span class="status-badge status-buff">💚 +${status.regen}/T</span>`);
    
    // 디버프 효과 (빨간색)
    if (status.vulnerable > 0) badges.push(`<span class="status-badge status-debuff">⚠️ 취약<sub>${status.vulnerable}T</sub></span>`);
