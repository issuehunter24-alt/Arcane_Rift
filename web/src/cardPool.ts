...
/**
 * 카드 UI 오브젝트 풀링 시스템
 * 매번 생성/삭제 대신 재사용하여 GC 부하 감소
 */

interface PooledCard {
  container: Container;
  sprite: Sprite | Graphics;
  costText: Container; // 코스트 (배경 원 + 텍스트)
  typeIcon: Container; // 카드 타입 아이콘 (배경 원 + 심볼)
  nameText: Text;
  inUse: boolean;
}
...
class CardPool {
   ...
   */
  acquire(cardWidth: number, cardHeight: number): PooledCard {
    // 사용 가능한 카드 찾기
   ...
   */
  replaceSprite(pooledCard: PooledCard, newSprite: Sprite | Graphics) {
   ...
   * 타입 아이콘 설정 (배경 원 + 심볼)
   */
  setupTypeIcon(pooledCard: PooledCard, backgroundColor: number, iconPath: string) {
    const container = pooledCard.container;
   ...
   * 코스트 텍스트 설정 (배경 원 + 텍스트)
   */
  setupCostText(pooledCard: PooledCard, cost: number, cardWidth: number, isAffordable: boolean) {
    const container = pooledCard.container;
